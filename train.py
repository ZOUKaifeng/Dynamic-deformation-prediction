...
def evaluate(coma, output_dir, test_loader, dataset, template_mesh, device, visualize=False):
    ...
    for i, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            e = torch.abs(out-data.y)
            error.append(e.cpu().numpy())
        total_loss += data.num_graphs * loss.item()
        ...
        if visualize and i % 100 == 0:
            ...
            save_obj(result_mesh.v, result_mesh.f+1, os.path.join(output_dir, 'file'+str(i)+'.obj'))
            save_obj(expected_mesh.v, expected_mesh.f+1, os.path.join(output_dir, 'gt'+str(i)+'.obj'))

            #meshviewer[0][0].save_snapshot(os.path.join(output_dir, 'file'+str(i)+'.png'), blocking=False)

    np.savez("./error_10.npz", err = error)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--conf', default = './default.cfg', help='path of config file')
